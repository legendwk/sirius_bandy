...
class Stats1:
    ...
    def __init__(self, teams: set, filename: str) -> None:
        ...
        self.df = gf.read_csv_as_df(filename)
    ...
    def create_instance_variables(self) -> None:
        ...
        self.shot_types = {'free': {x: list() for x in self.teams}, 'cross': {x: list() for x in self.teams}, 'outside': {x: list() for x in self.teams}, 
        'dribble': {x: list() for x in self.teams}, 'central': {x: list() for x in self.teams},},
    ...
    def get_shottypes(self, team: str) -> None:
    ...
    def get_possession(self) -> None:
        ...
        for i in range(len(self.possession_list) - 1):
            if self.possession_list[i][0] in self.teams: # this is the current team
                self.stats_dict['possession'][self.possession_list[i][0]] += gf.readable_to_sec(self.possession_list[i+1][1]) - gf.readable_to_sec(self.possession_list[i][1])
        for team in self.stats_dict['possession']:
            self.stats_dict['possession'][team] = gf.sec_to_readable(self.stats_dict['possession'][team])
        return
    ...
    def note_shottypes(self, row: pd.core.series.Series) -> None:
        '''checks row for shot types and updates the appropriate dictionaries'''
        if row['event'] == 'skottyp':
